...
These configs are **dev-stage** and reference the current catalog + prompt bank IDs. IDs may change as the curated catalog/prompt bank evolves.
...
- `sampling` controls model/persona mix and decode params; the resolved trial plan is written to `manifest.json` and trial assignments are recorded in `trials.jsonl`.
- `protocol` controls debate_v1 call structure and timeouts; prompt text is embedded into `config.resolved.json`.
